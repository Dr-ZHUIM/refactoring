...
function renderHTML(data: StatementData): LendRes {
  ...
  updateResult();
  result['statement'] += `<p>Amount owed is<em>${usd(
    data.totalAmount,
  )}</em></p>\n`;
  result[
    'statement'
  ] += `<p>You earned<em>${data.totalCredits}</em>credits</p>\n`;
  return result;
  ...
  function updateResult() {
    for (const perf of data.performances) {
      ...
      result['statement'] += `<tr><td>${perf.play.name}</td><td>${usd(
        perf.amount,
      )}</td>`;
